...
module.exports = (function (get, set, clear) {
	...
	theService.flush = (cb) => {
		collectionService.getResumeMarkers().deleteMany({selector: selector.normalized}, function(err, data) {
			if (ranges.length > 1) {
				...
				collectionService.getResumeMarkers().insertOne(range[0], function(err, data) { 
					if (err) throw err;
	...
	// this is for resume_marker

	// I don't think this is necessary.. its an artifact from before the rewrite. We no longer need to 
	//  select specific resumeMarkers from the DB. We read all of them, and deal with them in memory.

	// theService.getInitialQueryAttributes = function(startingPointTradeId, optsFunc) {
	// 	var opts = {};
