...
module.exports = (function (get, set, clear) {
	...
	function _createNewRange(obj) {
		var range = {from: obj.trade_id, to: obj.trade_id, oldest_time: obj.time, newest_time: obj.time};
	...
	theService.load = (cb) => {
		collectionService.getResumeMarkers().find().toArray(function (err, data) {
			ranges = data;
	...
	theService.flush = (cb) => {
		collectionService.getResumeMarkers().deleteMany({}, function(err, data) {
