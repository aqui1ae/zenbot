...
module.exports = function container (get, set, clear) {
  ...
  var productsService = get('lib.products-service')(get, set, clear)
  var periodService = get('lib.period-service')(get, set, clear)

  return function (s) {
    let so = s.options
    s.asset_capital = 0

    s.exchange = exchangeService.getExchange(so.selector.exchange_id)
    ...
    if (!productsService.getSelectedProduct()) {
      console.error('error: could not find product "' + so.selector.product_id + '"')
      process.exit(1)
    }

    if ((so.mode === 'live' || so.mode === 'paper') && s.exchange.dynamicFees) {
      s.exchange.setFees({asset: so.selector.asset, currency: so.selector.currency})
    }
    ...
    function fa (amt) {
      return n(amt).format('0.00000000') + ' ' + so.selector.asset
    }

    function isFiat () {
      return !so.selector.currency.match(/^BTC|ETH|XMR|USDT$/)
    }
    ...
    function fc (amt, omit_currency, color_trick, do_pad) {
      ...
      return str + (omit_currency ? '' : ' ' + so.selector.currency)
    }
    ...
    function onTrade (trade) {
      s.period = periodService.update(trade, so.period_length)
    ...
    function syncBalance (cb) {
      ...
      s.exchange.getBalance({currency: so.selector.currency, asset: so.selector.asset}, function (err, balance) {
        if (err) return cb(err)
        s.balance = balance
        s.exchange.getQuote({product_id: so.selector.product_id}, function (err, quote) {
          if (err) return cb(err)
          ...
          if (!s.start_capital) {
            ...
            pushMessage('Balance ' + s.exchange.name.toUpperCase(), 'sync balance ' + s.start_capital + ' ' + so.selector.currency  + '\n')
          }
    ...
    function placeOrder (type, opts, cb) {
      ...
      if (so.mode !== 'live') {
        ...
        order.product_id = so.selector.product_id
        ...
        s.exchange[type](order_copy, function (err, api_order) {
          ...
          function cancelOrder (do_reorder) {
            ...
            s.exchange.cancelOrder({order_id: order.order_id, product_id: so.selector.product_id}, function () {
              function checkHold () {
                s.exchange.getOrder({order_id: order.order_id, product_id: so.selector.product_id}, function (err, api_order) {
                  if (api_order) {
                    s.api_order = api_order
            ...
            s.exchange.getOrder({order_id: order.order_id, product_id: so.selector.product_id}, function (err, api_order) {
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                getQuote(function (err, quote) {
                  ...
                  let marked_price
                  let sProduct = productsService.getSelectedProduct()

                  if (type === 'buy') {
                    marked_price = n(quote.bid).subtract(n(quote.bid).multiply(so.markdown_buy_pct / 100)).format((so.currency_increment !== null) ? so.currency_increment : sProduct.increment, Math.floor)
                    ...
                    marked_price = n(quote.ask).add(n(quote.ask).multiply(so.markup_sell_pct / 100)).format((so.currency_increment !== null) ? so.currency_increment : sProduct.increment, Math.ceil)
                    if (n(order.price).value() > marked_price) {
                      msg(marked_price + ' vs our ' + order.price)
                      cancelOrder(true)
                    }
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            let sProduct = productsService.getSelectedProduct()
            price = n(quote.bid).subtract(n(quote.bid).multiply(so.markdown_buy_pct / 100)).format(sProduct.increment, Math.floor)
            if (!size) {
              if (so.mode === 'live' || so.mode === 'paper') {
            ...
            if ((sProduct.min_size && Number(size) >= Number(sProduct.min_size)) || ('min_total' in sProduct && sProduct.min_total && n(size).multiply(price).value() >= Number(sProduct.min_total))) {
              if (sProduct.max_size && Number(size) > Number(sProduct.max_size)) {
                size = sProduct.max_size
              }
            ...
            price = n(quote.ask).add(n(quote.ask).multiply(so.markup_sell_pct / 100)).format(sProduct.increment, Math.ceil)
            if (!size) {
              size = n(s.balance.asset).multiply(so.sell_pct / 100).format('0.00000000')
            }
            if ((sProduct.min_size && Number(size) >= Number(sProduct.min_size)) || (sProduct.min_total && n(size).multiply(price).value() >= Number(sProduct.min_total))) {
              if (sProduct.max_size && Number(size) > Number(sProduct.max_size)) {
                size = sProduct.max_size
              }
    ...
    function writeReport (is_progress, blink_off) {
      ...
      process.stdout.write('  ' + fc(s.period.close, true, true, true) + ' ' + so.selector.product_id.grey)
      if (s.lookback[0]) {
      ...
      if (orig_capital) {
        let asset_col = n(s.balance.asset).format(so.selector.asset === 'BTC' ? '0.00000' : '0.00000000') + ' ' + so.selector.asset
        asset_col_width = Math.max(asset_col.length + 1, asset_col_width)
        process.stdout.write(z(asset_col_width, asset_col, ' ').white)
        let currency_col = n(s.balance.currency).format(isFiat() ? '0.00' : '0.00000000') + ' ' + so.selector.currency
      ...
      update: function (trades, is_preroll, cb) {
        ...
        let tasks = trades.map(function (trade) {
          return function (done) {
            ...
            s.last_day = day
            if (!s.period) {
              s.period = periodService.init(trade, so.period_length)
            }
            s.in_preroll = is_preroll || (so.start && trade.time < so.start)
       
            // if the current trade has advanced us to a point 
            let period_id = periodService.getPeriodIdForTimeInMillis(trade.time);
            if (!periodService.isTimeWithinCurrentPeriod(trade.time)) {
