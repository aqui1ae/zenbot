let tb = require('timebucket')
  ...
  , rsi = require('./rsi')
  , productsService = require('../../lib/services/products-service')
  , periodService = require('../../lib/services/period-service')

let nice_errors = new RegExp(/(slippage protection|loss protection)/)

module.exports = function (s, conf) {
  var productsServiceInstance = productsService(conf)
  var periodServiceInstance = periodService(conf)
  ...
  if (!productsServiceInstance.getSelectedProduct()) {
  ...
  function onTrade (trade) {
    periodServiceInstance.update(trade)
  ...
  function executeStop (do_sell_stop) {
    let currentPeriod = periodServiceInstance.getCurrentPeriod()
    let stop_signal
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    syncBalance(function (err) {
      ...
      getQuote(function (err, quote) {
        ...
        let selectedProduct = productsServiceInstance.getSelectedProduct()
  ...
  function writeReport (is_progress, blink_off) {
    ...
    process.stdout.write(moment(is_progress ? currentPeriod.close_time : tb(currentPeriod.time).resize(so.period_length).add(1).toMilliseconds()).format('YYYY-MM-DD HH:mm:ss')[is_progress && !blink_off ? 'bgBlue' : 'grey'])
    process.stdout.write('  ' + fc(currentPeriod.close, true, true, true) + ' ' + s.product_id.grey)

    // TODO: Fire event here.
    var lookback = periodServiceInstance.getMostRecentlyPushedLookbackPeriod()
    if (lookback) {
    ...
    update: function (trades, is_preroll, cb) {
      ...
      let tasks = trades.map(function (trade) {
        ...
        return function (done) {
          ...
          s.last_day = day
          if (!currentPeriod) {
            periodServiceInstance.init(trade)
          }
          ...
          if (period_id !== currentPeriod.period_id) {
            s.strategy.onPeriod.call(s.ctx, s, function () {
              ...
              periodServiceInstance.pushCurrentPeriodToLookback()
              periodServiceInstance.init(trade)
