...
module.exports = function (s, conf, services) {
  var productsService = services.productsService
  var periodService = services.periodService
  var selectorService = services.selectorService
  var selector = selectorService.getSelector()

  let so = s.options
  s.exchange = require(path.resolve(__dirname, `../extensions/exchanges/${selector.exchange_id}/exchange`))(conf)
  s.asset_capital = 0

  

  periodService.setOptions({period_length: so.period_length || so.period})

  if (!productsService.getSelectedProduct()) {
    console.error('error: could not find product "' + selector.product_id + '"')
    process.exit(1)
  }

  if (so.strategy) {
    s.strategy = require(path.resolve(__dirname, `../extensions/strategies/${so.strategy}/strategy`))(services)
  ...
  if ((so.mode === 'live' || so.mode === 'paper') && s.exchange.dynamicFees) {
    s.exchange.setFees({asset: selector.asset, currency: selector.currency})
  }
  ...
  function fa (amt) {
    return n(amt).format('0.00000000') + ' ' + selector.asset
  }

  function isFiat () {
    return !selector.currency.match(/^BTC|ETH|XMR|USDT$/)
  }
  ...
  function fc (amt, omit_currency, color_trick, do_pad) {
    ...
    return str + (omit_currency ? '' : ' ' + selector.currency)
  }
  ...
  function executeStop (do_sell_stop) {
    let currentPeriod = periodService.getCurrentPeriod()
    let stop_signal
    if (s.my_trades.length || s.my_prev_trades.length) {
      ...
      s.last_trade_worth = last_trade.type === 'buy' ? (currentPeriod.close - last_trade.price) / last_trade.price : (last_trade.price - currentPeriod.close) / last_trade.price
      if (!s.acted_on_stop) {
        if (last_trade.type === 'buy') {
          if (do_sell_stop && s.sell_stop && currentPeriod.close < s.sell_stop) {
            stop_signal = 'sell'
            ...
            s.profit_stop_high = Math.max(s.profit_stop_high || currentPeriod.close, currentPeriod.close)
            s.profit_stop = s.profit_stop_high - (s.profit_stop_high * (so.profit_stop_pct / 100))
          }
          if (s.profit_stop && currentPeriod.close < s.profit_stop && s.last_trade_worth > 0) {
            stop_signal = 'sell'
          ...
          if (s.buy_stop && currentPeriod.close > s.buy_stop) {
            stop_signal = 'buy'
  ...
  function syncBalance (cb) {
    ...
    s.exchange.getBalance({currency: selector.currency, asset: selector.asset}, function (err, balance) {
      if (err) return cb(err)
      s.balance = balance
      s.exchange.getQuote({product_id: productsService.getSelectedProductId()}, function (err, quote) {
        if (err) return cb(err)
        ...
        if (!s.start_capital) {
          ...
          pushMessage('Balance ' + s.exchange.name.toUpperCase(), 'sync balance ' + s.start_capital + ' ' + selector.currency  + '\n')
        }
  ...
  function placeOrder (type, opts, cb) {
    ...
    if (so.mode !== 'live') {
      if (!order.orig_time) order.orig_time = currentPeriod.close_time
      order.time = currentPeriod.close_time
      return cb(null, order)
    }
    else {
      order.product_id = productsService.getSelectedProductId()
      ...
      s.exchange[type](order_copy, function (err, api_order) {
        ...
        function cancelOrder (do_reorder) {
          ...
          s.exchange.cancelOrder({order_id: order.order_id, product_id: productsService.getSelectedProductId()}, function () {
            function checkHold () {
              s.exchange.getOrder({order_id: order.order_id, product_id: productsService.getSelectedProductId()}, function (err, api_order) {
                if (api_order) {
                  s.api_order = api_order
  ...
  function getQuote (cb) {
    ...
    if (so.mode === 'sim' || so.mode === 'train') {
      return cb(null, {
        bid: currentPeriod.close,
        ask: currentPeriod.close
      })
    }
    else {
      s.exchange.getQuote({product_id: productsService.getSelectedProductId()}, function (err, quote) {
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    syncBalance(function (err) {
      ...
      getQuote(function (err, quote) {
        ...
        let selectedProduct = productsService.getSelectedProduct()

        if (signal === 'buy') {
          price = n(quote.bid).subtract(n(quote.bid).multiply(so.markdown_buy_pct / 100)).format(selectedProduct.increment, Math.floor)
          if (!size) {
            if (so.mode === 'live' || so.mode === 'paper') {
          ...
          if ((selectedProduct.min_size && Number(size) >= Number(selectedProduct.min_size)) || ('min_total' in selectedProduct && selectedProduct.min_total && n(size).multiply(price).value() >= Number(selectedProduct.min_total))) {
            if (selectedProduct.max_size && Number(size) > Number(selectedProduct.max_size)) {
              size = selectedProduct.max_size
            }
          ...
          price = n(quote.ask).add(n(quote.ask).multiply(so.markup_sell_pct / 100)).format(selectedProduct.increment, Math.ceil)
          if (!size) {
            size = n(s.balance.asset).multiply(so.sell_pct / 100).format('0.00000000')
          }
          if ((selectedProduct.min_size && Number(size) >= Number(selectedProduct.min_size)) || (selectedProduct.min_total && n(size).multiply(price).value() >= Number(selectedProduct.min_total))) {
            if (selectedProduct.max_size && Number(size) > Number(selectedProduct.max_size)) {
              size = selectedProduct.max_size
            }
  ...
  function writeReport (is_progress, blink_off) {
    let currentPeriod = periodService.getCurrentPeriod()
    let currentPeriodLength = periodService.getPeriodLength()
    ...
    process.stdout.write(moment(is_progress ? currentPeriod.close_time : tb(currentPeriod.time).resize(currentPeriodLength).add(1).toMilliseconds()).format('YYYY-MM-DD HH:mm:ss')[is_progress && !blink_off ? 'bgBlue' : 'grey'])
    process.stdout.write('  ' + fc(currentPeriod.close, true, true, true) + ' ' + productsService.getSelectedProductId().grey)

    // TODO: Fire event here.
    var lookback = periodService.getMostRecentlyPushedLookbackPeriod()
    if (lookback) {
      let diff = (currentPeriod.close - lookback.close) / lookback.close
    ...
    let volume_display = currentPeriod.volume > 99999 ? abbreviate(currentPeriod.volume, 2) : n(currentPeriod.volume).format('0')
    ...
    let rsi_periods = periodService.getOption('rsi_periods')
    rsi(s, 'rsi', periodService, rsi_periods)
    if (typeof currentPeriod.rsi === 'number') {
      let half = 5
      let bar = ''
      let stars = 0
      let rsi = n(currentPeriod.rsi).format('00.00')
      if (currentPeriod.rsi >= 50) {
        stars = Math.min(Math.round(((currentPeriod.rsi - 50) / 50) * half) + 1, half)
        ...
        stars = Math.min(Math.round(((50 - currentPeriod.rsi) / 50) * half) + 1, half)
        bar += ' '.repeat(half - stars)
    ...
    if (orig_capital) {
      let asset_col = n(s.balance.asset).format(selector.asset === 'BTC' ? '0.00000' : '0.00000000') + ' ' + selector.asset
      asset_col_width = Math.max(asset_col.length + 1, asset_col_width)
      process.stdout.write(z(asset_col_width, asset_col, ' ').white)
      let currency_col = n(s.balance.currency).format(isFiat() ? '0.00' : '0.00000000') + ' ' + selector.currency
      ...
      let consolidated = n(s.balance.currency).add(n(currentPeriod.close).multiply(s.balance.asset)).value()
      let profit = (consolidated - orig_capital) / orig_capital
      process.stdout.write(z(8, pct(profit), ' ')[profit >= 0 ? 'green' : 'red'])
      let buy_hold = currentPeriod.close * (orig_capital / orig_price)
    ...
    update: function (trades, is_preroll, cb) {
      ...
      let tasks = trades.map(function (trade) {
        ...
        return function (done) {
          ...
          if (currentPeriod && trade.time < currentPeriod.time) {
            return done()
          }
          let period_id = tb(trade.time).resize(currentPeriodLength).toString()
          ...
          s.last_day = day
          if (!currentPeriod) {
            periodService.init(trade)
          }
          s.in_preroll = is_preroll || (so.start && trade.time < so.start)
          if (period_id !== currentPeriod.period_id) {
            s.strategy.onPeriod.call(s.ctx, s, function () {
              ...
              periodService.pushCurrentPeriodToLookback()
              periodService.init(trade)
